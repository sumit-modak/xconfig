...
if [[ $SYS == "Y" || $SYS == "y" ]]; then
  sudo pacman -S man zip unzip patch curl wget \
...
if [[ $GUI == "Y" || $GUI == "y" ]]; then
  sudo pacman -S xorg xserver-xorg picom xwallpaper maim \
...
if [[ $VIRT == "Y" || $VIRT == "y" ]]; then
  sudo pacman -S qemu-utils qemu-system-x86 qemu-system-gui \
...
if [[ $UTIL == "Y" || $UTIL == "y" ]]; then
  sudo pacman -S htop cmatrix neofetch figlet \
